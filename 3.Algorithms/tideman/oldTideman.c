...
{ 
    ...
    { 
        // If the next candidate is the starting one, cycle found 
        if (locked[current][next] == 1 && next == start) 
        { 
            return 1; 
        } 
        // If the current candidate is a winner, chain continues 
        else if (locked[current][next] == 1) 
        { 
            bool cycle = cycle_found(start, next); 
            if (cycle == 1) 
            { 
                return 1; 
            } 
    ...
    // If no matches are found, chain ended 
    return 0; 
}
